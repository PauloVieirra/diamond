...
const Desempenho = () => {
  ...
  useEffect(() => {
    if (disciplinaSelecionada) {
      const filtrados = assuntos.filter(assunto => assunto.disciplina_id === parseInt(disciplinaSelecionada, 10));
  ...
  const fetchAssuntos = async () => {
    const { data, error } = await supabase.from('assuntos').select('id, nome, disciplina_id');
    if (error) {
  ...
  const handleSave = async () => {
    if (!disciplinaSelecionada || !assuntoSelecionado || !desempenho || !tipoDesempenho || !titulo) {
  ...
  return (
    <div className={`desempenho-page ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <button onClick={handleSave}>Salvar Desempenho</button>
  ...
  <p>Nenhum desempenho salvo para esse parâmetro. Mude o parâmetro ou crie um desempenho para esse.</p>
)}
